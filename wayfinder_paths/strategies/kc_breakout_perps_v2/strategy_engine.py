...
class Result:
    ...
    # per-bar ATR stop DISTANCE (d = ATR*atr_volatility_factor) of the active
    # trade, held constant while in the trade, NaN when flat. decide() uses this
    # plus the live fill price to place a native reduce-only stop-loss trigger:
    # stop = entry - d (long). Surfaced via SignalFrame.extras["stop_distance"].
    stop_distance: pd.Series = field(default_factory=lambda: pd.Series(dtype=float))
...
def run(df: pd.DataFrame, cfg: Config, funding: pd.Series | None = None,
    ...
    for i in range(eval_start, hi):
            ...
            if d > 0 and np.isfinite(d):
                ...
                if notional >= cfg.min_notional and size > 0:
                    ...
                    pos = {"side": side, "entry_idx": i, "entry_price": entry, "size": size,
                           "stop": stop, "tp": tp, "d": d, "fees": entry_fee, "funding": 0.0,
        ...
        sd_vals.append(pos["d"] if pos is not None else np.nan)
    ...
    sd = pd.Series(sd_vals, index=idx, name="stop_distance")
    return Result(trades=trades, equity=eq, stats=compute_stats(eq, trades, cfg),
                  weights=w, stop_distance=sd)
