...
async def fetch_latest_rates() -> dict:
    ...
    # Latest price
    prices = virtual_ts.get("price", [])
    price = prices[-1]["price_usd"] if prices else 0

    # Latest funding (pick highest OI venue)
    funding_points = virtual_ts.get("funding", [])
    funding_rate = 0.0
    if funding_points:
        best = max(funding_points, key=lambda f: f.get("oi_usd", 0))
        funding_rate = best.get("funding_rate", 0)

    # Latest Moonwell VIRTUAL supply APR
    virtual_supply_apr = 0.0
    for lp in reversed(virtual_ts.get("lending", [])):
    ...
    usdc_supply_apr = 0.0
    for lp in reversed(usdc_ts.get("lending", [])):
        venue = (lp.get("venue") or "").lower()
        if "moonwell" in venue:
