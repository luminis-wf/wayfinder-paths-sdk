...
The Path manifest is the source-of-truth for publishing a Path bundle.
...
Notes:
- Prefer `applet/dist` (not a top-level `dist/`) so the path bundler doesn’t accidentally exclude your UI build output.
...
If the path should export AI skill artifacts, add an explicit `skill` block.
...
Rules:
- `enabled: true` opt-ins the path to skill validation and rendering.
