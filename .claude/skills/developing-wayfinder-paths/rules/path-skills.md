...
Wayfinder paths should treat skill support as explicit manifest behavior, not as an ad hoc `skill/SKILL.md` file that authors hand-maintain for every host.

## Canonical source

For normal paths, the source of truth is:
...
skill:
  ...
  source: generated
  name: "my-path"
  description: "Inspect, validate, and operate the path."
  instructions: "skill/instructions.md"
```

The `instructions` file should contain prose only. Keep host-specific metadata in `wfpath.yaml`.
...
- `portable/<skill-name>/scripts/wf_bootstrap.py`
- `portable/<skill-name>/scripts/wf_run.py`

Optional `skill/scripts`, `skill/references`, and `skill/assets` folders are copied into each host export.

## Validation

`wayfinder path doctor --check --path .` enforces the common cross-host contract:
...
`wayfinder path doctor --fix --path .` can scaffold missing generated-mode stubs like `skill/instructions.md`, but it does not invent missing required manifest fields.
...
`wayfinder path fmt --path .` is intentionally artifact-oriented.
